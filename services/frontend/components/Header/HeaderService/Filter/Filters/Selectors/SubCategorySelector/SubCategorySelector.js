...
import {adaptiveSelect, backDropper, selectArrow, selectScroll, selectScrollOptions, categoryTitle} from "../../../filter.module.scss"
...
const SubCategorySelector = ({holder, name, items, disabled, zIndex}) => {
    ...
    return (
        <>
            <div className={adaptiveSelect} style={isOpen ? {...selctedStyle, marginTop: 12} : {marginTop: 12}} >
                <button onClick={onSwitchOpen} {...{disabled}}>
                    <Text content={nameValue ? selectedValue?.text || holder : holder} />
                </button>

                <svg className={selectArrow} onClick={disabled ? () => null: onSwitchOpen} viewBox="0 0 18 10" fill="none" xmlns="http://www.w3.org/2000/svg" style={isOpen ? {transform: 'rotate(180deg)'} : null}>
                        ...
                        <div className={backDropper}>
                            ...
                            <div className={selectScroll} style={{zIndex: 2}}>
                                <div ref={scrollElement} className={selectScrollOptions}  >
                                    ...
                                    {   
                                        items?.map(({text, value}, index) => <SubCategoryOption key={index} 
