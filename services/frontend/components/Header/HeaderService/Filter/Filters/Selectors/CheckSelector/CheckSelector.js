...
import {select, selectOptions, selectArrow, selectTitle, selectDimmer, applyBtn, adaptiveCheckSelct} from "../../../filter.module.scss"
...
const CheckSelector = ({holder, name, items, zIndex}) => {
    ...
    return (
        <>
            <div className={adaptiveCheckSelct} style={isOpen ? selctedStyle : null}>
                <button onClick={onSwitchOpen}>
                    {!!nameValue?.length ? (nameValue instanceof Array ? nameValue.map((value, index) => {
                        return <Relative key={index}><Text content={value} />{index === nameValue.length-1 ? '' : ', '}</Relative>
                    }) 
                    : <Text content={nameValue} />) 
                    : <Text content={holder} />}
                </button>
                ...
                
                {
                    isOpen && <>
                        ...
                        <div className={selectOptions} style={{zIndex}} {...swipe}>
                            <div className="close-line close-line--top" onClick={onSwitchOpen} />
                            <p className={selectTitle}><Text content={holder} /></p>
                            {   
                            ...
                            <a className={applyBtn} onClick={onSwitchOpen}>
                                <Text content="Apply" />
                            </a>
                        </div>
                    </>
