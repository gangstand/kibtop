...
import {optionCategory} from "../../../filter.module.scss"


const CategoryOption = ({text, value, isSelected, register, onClick}) => {

    return (
        <>
            <label style={{
                color: isSelected ? '#65A4FE' : 'inherit'
            }} {...{onClick, className: optionCategory}}>
                <input {...register} hidden={true} value={value} type="radio" />
                <img src={`/img/add_advert/${!!value ? value : 'all'}.png`} alt="" className="category-name__img" />
